	...
	Properties{
		...
		SubShader{
			...
			void vert(inout appdata_full data) {
				...
				modifiedPos.y += sin(data.vertex.z * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;

				float3 posPlusTangent = data.vertex + data.tangent * 0.01;
				posPlusTangent.y += sin(posPlusTangent.z * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;

				float3 bitangent = cross(data.normal, data.tangent);
				float3 posPlusBitangent = data.vertex + bitangent * 0.01;
				posPlusBitangent.y += sin(posPlusBitangent.z * _Frequency + _Time.y * _AnimationSpeed) * _Amplitude;
